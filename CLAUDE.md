...
- **老師只改得動自己寫的那幾筆**。刪掉家長誤傳的東西是管理,改掉家長寫的字是
  替他改口,那是兩回事,規則裡也是這樣寫的
...
- 時限在 `firestore.rules` 裡也擋一次(`request.time < createdAt + duration.value(15,'m')`),
  前端不給按只是裝飾
- **老師不受時限**。他要管理整個工作室的紀錄,包含刪掉測試用的簿子與家長誤傳的東西
...
- 刪除一律先問一次,而且問句就長在那一筆上(不用 `confirm()`),手機上比較不會誤按。
  只有「刪掉整本」用 `confirm()` —— 那個真的不能按錯
- **刪紀錄要先收附件再刪本體**。順序反了的話 media 文件會變成沒人指得到的孤兒
- Firestore **刪父文件不會連子集合一起刪**。刪整本簿子要自己把 entries 與 media
  一筆一筆收掉
- 老師刪得掉家長寫的那幾筆(規則有開)。不然測試用的簿子永遠刪不乾淨
...
10. 把一筆的 createdAt 往前推,家長那邊「改」與「刪」都要消失,但作業還是打得了勾;
    老師自己的舊紀錄仍改得動,且改不動家長寫的字
11. CDN 連不上之後切一次語言,署名鈕必須還是停用的(`unavailable` 旗標,踩過)
