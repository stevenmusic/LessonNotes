...
名字隨時可以改，家長改自己的，老師都能改。**誰都碰不到別人寫的東西，老師也一樣。** 家長的請假、回報、申訴是他自己的紀錄，
不該被另一方修改或抹掉。**家長**改寫與刪除自己那幾筆限 15 分鐘內；
**老師**改寫與刪除自己那幾筆不受限（需要事後改正的是他：小節數寫錯、譜名打錯，
家長已經照著練了還是得改對）。改過的紀錄一律標上「已修改」。

因為紀錄只有作者本人刪得掉，簿本**不能整本刪除**（會留下刪不掉的孤兒資料）。
不再上課的學生、測試用的簿本改成**封存**：從名冊上收起來，紀錄原封不動，
隨時可以取回。
